...
let previousWinners = [];
let tokenSupply = 0;
let majorHolders = new Map(); // wallet -> {tokens, percentage}
        ...
        for (const account of largestAccounts.value) {
            try {
                ...
                if (accountInfo && accountInfo.value) {
                    ...
                    const accountInfoDetailed = await connection.getAccountInfo(account.address);
                    if (accountInfoDetailed && accountInfoDetailed.data) {
                        // Token account owner is at byte offset 32-64 in the data
                        const ownerPubkey = new PublicKey(accountInfoDetailed.data.subarray(32, 64));
...
function processGameBlock(purchase) {
    ...
    if (purchase.isMillionTokenHolder) {
        blocksToOpen = Math.max(blocksToOpen, 1); // At least 1 block for million token holders
    ...
    if (actualBlocksToOpen > 0) {
        ...
        for (let i = 0; i < actualBlocksToOpen; i++) {
            ...
            if (purchase.isMillionTokenHolder && i === 0) {
                // First block for million token holders is guaranteed green
                blockColor = 'green';
            } else {
                // Regular blocks have 33% chance for green
