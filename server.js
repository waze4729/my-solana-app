...
function getRandomTTL() {
  return 180 + Math.floor(Math.random() * (320 - 180));
}
...
// Core polling now only updates top holders for this token, nothing about their other SPL tokens
async function pollData() {
  ...
  try {
    ...
    // No more SPL token fetching, just use holder data
    storage.latestData = {
      fresh,
      ...
      topHolders: topHoldersRaw // No valuableTokens here
    };
  } catch (error) {
...
// Removed pollTopHolderSplTokens and all walletTokenCache logic
